...
# ==========================================
# 2022 澳門四校聯考 · 數學附加卷 (5 題)
# ==========================================
sup_2022_slides = [
    # Q1
    {
        "year": "2022", "paper": "附加卷", "qNum": "第1題", "topic": "空間立體幾何 · 三棱錐體積、垂直證明與二面角", "score": "20分",
        ...
        "knowledge": {
            "formulas": ["PC \\perp \\text{面 } ABC \\implies PC \\perp BC, \\; PC \\perp AC", "DE \\text{ 為 } \\triangle ABC \\text{ 中位線} \\implies DE \\parallel AC, \\; DE = \\frac{1}{2}AC", "\\text{二面角平面角：三垂線定理或法向量夾角}"],
            "points": ["<b>中位線平行性質</b>：$D, E$ 為中點，則 $DE \\parallel AC$ 且 $DE = \\frac{1}{2}AC = \\sqrt{3}$。", "<b>線面垂直判定</b>：$AC \\perp BC \\implies DE \\perp BC$；又 $PC \\perp$ 面 $ABC \\implies PC \\perp DE$。故 $DE \\perp$ 面 $PBC$！", "<b>等腰三角形對稱性</b>：利用 $|PE| = |PD|$ 求得邊長 $|BC|$。"],
    ...
    # Q2 with VISUAL SLIDER! (Cubic curve tangent and extrema)
    {
        "year": "2022", "paper": "附加卷", "qNum": "第2題", "topic": "微積分 · 函數零點、極值拐點與割線定積分面積", "score": "20分",
        ...
        "knowledge": {
            "formulas": ["f(x) = (x - 1)^2 (x + 2) = 0", "f'(x) = 3x^2 - 3 = 3(x - 1)(x + 1), \\quad f''(x) = 6x", "S = \\int_{-1}^2 [4 - (x^3 - 3x + 2)] dx = \\frac{27}{4}"],
            "points": ["<b>因式分解重根特徵</b>：$x = 1$ 是二重切點根，$x = -2$ 為單根。", "<b>極值與拐點</b>：極大值 $f(-1) = 4$，極小值 $f(1) = 0$；拐點為 $(0, 2)$。", "<b>水平割線交點求定積分面積</b>：聯立 $x^3 - 3x + 2 = 4 \\iff (x + 1)^2 (x - 2) = 0$。積分區間為 $[-1, 2]$。"],
    ...
    # Q3
    {
        "year": "2022", "paper": "附加卷", "qNum": "第3題", "topic": "平面解析幾何 · 向量點積軌跡方程與割線弦長", "score": "20分",
        ...
        "knowledge": {
            "formulas": ["\\vec{PA} = (-1-x, -y), \\; \\vec{PB} = (1-x, -y)", "\\vec{PA} \\cdot \\vec{PB} = x^2 - 1 + y^2 = -\\frac{1}{2}y^2 \\implies x^2 + \\frac{3}{2}y^2 = 1 \\iff x^2 + \\frac{y^2}{2/3} = 1"],
            "points": ["<b>向量坐標數量積</b>：$(-1-x)(1-x) + (-y)(-y) = x^2 - 1 + y^2$。", "<b>軌跡方程化為標準橢圓</b>：$x^2 + \\frac{3}{2}y^2 = 1$（長軸在 $x$ 軸上的橢圓）。", "<b>中點在 $y$ 軸上</b>：$x_M + x_N = 0$，韋達定理一次項係數為 0。"],
    ...
    # Q4
    {
        "year": "2022", "paper": "附加卷", "qNum": "第4題", "topic": "複數模長運算、二項式與棣莫弗定理", "score": "20分",
        ...
        "knowledge": {
            "formulas": ["|x - 2 + yi|^2 = |x + (y + 2)i|^2 \\implies (x-2)^2 + y^2 = x^2 + (y+2)^2", "w = e^{i\\pi/3} \\implies w^3 = e^{i\\pi} = -1, \\; w^6 = 1", "w^2 - w + 1 = 0 \\implies 1 - w + w^2 = 0"],
            "points": ["<b>複數距離幾何意義</b>：點 $z$ 到點 $(2, 0)$ 的距離等於到點 $(0, -2)$ 的距離，軌跡為兩點垂直平分線 $y = -x \\iff x + y = 0$。", "<b>三次單位根性質</b>：$w = \\cos\\frac{\\pi}{3} + i\\sin\\frac{\\pi}{3}$，$w^2 - w + 1 = 0$。"],
    ...
    # Q5
    {
        "year": "2022", "paper": "附加卷", "qNum": "第5題", "topic": "數學歸納法與矩陣高階次冪", "score": "20分",
        ...
        "knowledge": {
            "formulas": ["A^k \\cdot A = \\begin{pmatrix} 1 & k \\\\ 0 & 1 \\end{pmatrix}\\begin{pmatrix} 1 & 1 \\\\ 0 & 1 \\end{pmatrix} = \\begin{pmatrix} 1 & k+1 \\\\ 0 & 1 \\end{pmatrix}", "B = 2I + N \\implies B^n = 2^n I + n 2^{n-1} N = \\begin{pmatrix} 2^n & n 2^{n-1} \\\\ 0 & 2^n \\end{pmatrix}"],
            "points": ["<b>數學歸納法三步規範</b>：奠基步（$n=1$ 成立）、歸納假設（設 $n=k$ 成立）、遞推步（推導 $n=k+1$ 成立）。", "<b>矩陣二項式展開（Jordan塊）</b>：$B = 2A$ 或拆為純量陣加冪零陣。$B^n = 2^n A^n$。"],
...
# Write demo/ch-2022-supp.js
js_content = """/* 2022 澳門四校聯考 · 數學附加卷 (5 題全) */
(function() {
...
slides_str = "[\n"
for i, s in enumerate(sup_2022_slides):
    has_vis = "visual" in s
    vis_val = s.pop("visual") if has_vis else None
    s_json = json.dumps(s, ensure_ascii=False, indent=6)
    if has_vis:
        s_json = s_json[:-1].rstrip() + f',\n      "visual": {vis_val}\n    }}'
    slides_str += "      " + s_json + (",\n" if i < len(sup_2022_slides)-1 else "\n")

js_content += slides_str + "    ]\n  });\n})();\n"

with open("demo/ch-2022-supp.js", "w", encoding="utf-8") as f:
    f.write(js_content)

print("Successfully generated demo/ch-2022-supp.js with 5 slides!")
