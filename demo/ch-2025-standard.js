...
(function() {
  ...
  DECK.push({
    ...
    slides: [
      ...
      "knowledge": {
            "formulas": [
                  "x^2 + 3x - 4 = (x+4)(x-1) \\ge 0 \\iff x \\le -4 \\text{ 或 } x \\ge 1",
                  "A \\cap B = \\{x \\in B : x \\in A\\}"
            ],
            "points": [
                  "<b>交集定義</b>：屬於集合 $A$ 且屬於集合 $B$ 的所有元素之集合。",
                  "<b>二次不等式求法</b>：因式分解找出零點，大於取兩邊、小於取中間。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "\\text{韋達定理：若 } x_1, x_2 \\text{ 為 } ax^2+bx+c=0 \\text{ 之根，則 } x_1+x_2 = -\\frac{b}{a}, \\; x_1 x_2 = \\frac{c}{a}",
                  "2^{\\alpha+1} \\times 2^{\\beta+1} = 2^{\\alpha + \\beta + 2}"
            ],
            "points": [
                  "<b>指數運算法則</b>：同底數冪相乘，底數不變，指數相加。",
                  "<b>對稱代數式轉化</b>：先由根之和、積求出 $\\alpha\\beta$ 與 $\\alpha+\\beta$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "V = \\pi r^2 h",
                  "V_{\\text{新}} = \\pi (r(1+30\\%))^2 (h(1-30\\%)) = \\pi r^2 h \\times (1.3)^2 \\times 0.7"
            ],
            "points": [
                  "<b>體積對各變量的敏感度</b>：半徑是平方項 $r^2$，高是一次方項 $h$。",
                  "<b>增減百分比計算</b>：增長率 $= \\frac{V_{\\text{新}} - V}{V} \\times 100\\%$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "m^{\\frac{3}{2}} = m^{-\\frac{1}{2}} \\cdot m^2",
                  "m^2 - 1 = (m - 1)(m + 1)"
            ],
            "points": [
                  "<b>公因式提取法</b>：分子與分母均提取最低次冪 $m^{-\\frac{1}{2}}$。",
                  "<b>有理化/平方差公式</b>：約去公因式後利用平方差化簡。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "2^p = 5 \\iff p = \\log_2 5, \\quad 2^q = 7 \\iff q = \\log_2 7",
                  "\\log_a \\frac{x}{y} = \\log_a x - \\log_a y, \\quad \\log_a (xy) = \\log_a x + \\log_a y"
            ],
            "points": [
                  "<b>小數轉分數</b>：$0.7 = \\frac{7}{10} = \\frac{7}{2 \\times 5}$。",
                  "<b>對數真數拆解</b>：拆解為已知底數 2 與條件值 5, 7。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  ...
                  "A < B < C \\iff \\begin{cases} B > A \\\\ B < C \\end{cases}"
            ],
            "points": [
                  "<b>絕對值雙向展開</b>：拆成不等式組連立求解。",
                  "<b>二次不等式因式分解</b>：分別求兩側不等式之交集。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "P_n^n = n!, \\quad P_m^k = \\frac{m!}{(m-k)!}",
                  "\\text{插空法總數} = (\\text{主體全排列}) \\times (\\text{空位選取排列})"
            ],
            "points": [
                  "<b>不相鄰典型策略（插空法）</b>：先排沒有限制的元素（女生），形成首尾及相鄰間的空位，再將受限元素（男生）插入空位。",
                  "4 個女生排成一列產生 $4 + 1 = 5$ 個可插入的空位。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "\\text{中位數：偶數個數據排序後中間兩數之平均值}",
                  "\\text{平均值 } \\bar{x} = \\frac{\\sum x_i}{n}"
            ],
            "points": [
                  "<b>中位數求解關鍵</b>：必須先將數據由小到大嚴格排序！",
                  "<b>代數對稱性</b>：觀察各項常數項之和與中間項之關係。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "(x+y)^n \\text{ 通項 } T_{r+1} = \\binom{n}{r} x^{n-r} y^r"
            ],
            "points": [
                  "<b>乘積展開式特定項原理</b>：乘法分配律拆為兩部分分別找對應項。",
                  "第一部分：$x \\cdot [\\text{展開式中的 } x^3 y^5]$；第二部分：$\\frac{y^3}{x^2} \\cdot [\\text{展開式中的 } x^6 y^2]$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "\\text{圓標準方程：}(x-2)^2 + y^2 = 4 \\implies \\text{圓心 } C(2, 0), \\; \\text{半徑 } r = 2",
                  "\\text{半弦長公式：}\\left(\\frac{L}{2}\\right)^2 + d^2 = r^2 \\implies L = 2\\sqrt{r^2 - d^2}"
            ],
            "points": [
                  "<b>弦長極值原理</b>：弦長 $L = 2\\sqrt{r^2 - d^2}$，當圓心到直線距離 $d$ 達到最大時，弦長 $L$ 取得最小值。",
                  "<b>定點距離幾何性質</b>：過圓內定點 $P$ 的所有直線中，當且僅當直線垂直於 $CP$ 時，$d = |CP|$ 達到最大值。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "x^2 = -2py' \\text{ 或標準型 } x^2 = -4cy \\implies \\text{焦點 } \\left(0, -\\frac{p}{4}\\right), \\; \\text{準線 } y = \\frac{p}{4}",
                  "\\text{拋物線定義：}|AF| = d(A, \\text{準線})"
            ],
            "points": [
                  "<b>拋物線開口方向</b>：$x^2 = -py$ ($p>0$) 開口向下，位於 $x$ 軸下方 ($y \\le 0$)。",
                  "<b>到準線之距離</b>：若點 $A(x_0, y_0)$，則 $y_0 = -7$；準線為 $y = \\frac{p}{4}$，距離為 $\\frac{p}{4} - (-7) = \\frac{p}{4} + 7$。"
            ],
      ...
      "knowledge": {
            ...
            "points": [
                  "<b>互斥事件加法公式</b>：命中總數為 4 可拆分為 3 種互斥情形：(約翰1次, 安娜3次)、(約翰2次, 安娜2次)、(約翰3次, 安娜1次)。",
                  "<b>獨立事件乘法公式</b>：每種情形各自概率相乘。"
            ],
      ...
      "knowledge": {
            ...
            "points": [
                  "<b>基本量法</b>：將 $S_2, S_3, S_4$ 全用首項 $a_1 = 4$ 與公差 $d$ 表示。",
                  "<b>解方程求公差</b>：展開等式求出 $d$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "z = 3x + y \\implies y = -3x + z \\; (z \\text{ 為截距相關量})",
                  "\\text{凸多邊形頂點最值定理：線性目標函數最值必在可行域頂點取得}"
            ],
            "points": [
                  "<b>頂點坐標求解法</b>：聯立兩兩直線方程求出交點頂點。",
                  "<b>代入比較法</b>：將所有頂點坐標代入 $z = 3x+y$，最大者即為答案。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "f(x+2) = f(x) \\implies T = 2",
                  "x \\in [0, 1] \\implies x+4 \\in [4, 5] \\implies f(x) = f(x+4) = 1 + |(x+4)-5| = 2 - x"
            ],
            "points": [
                  "<b>區間平移化簡</b>：由週期為 2，將輸入的三角函數值（落在 $[-1, 1]$ 內）映射到已知區間 $[4, 6]$。",
                  "<b>單調性分析</b>：當 $t \\in [0, 1]$ 時，$f(t) = 2 - t$ 為嚴格單調遞減函數！自變量越大，函數值越小。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  ...
                  "T_n = \\sum (An+B) q^{n-1} \\implies \\text{乘公比 } q T_n \\text{ 錯位相減}"
            ],
            "points": [
                  "<b>$a_n$ 與 $S_n$ 關係</b>：務必分別驗證 $n=1$ 與 $n \\ge 2$ 的一致性。",
                  "<b>等差乘等比求和（錯位相減法）</b>：兩式相減後中段形成純等比數列，利用等比求和公式收斂。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "\\text{最高次與常數項匹配：} 2p = 8, \\; 3r = 9",
                  "\\text{餘式定理：} f(-1) = -10"
            ],
            "points": [
                  "<b>待定係數法</b>：對比兩端多項式最高次項係數與常數項迅速確定 $p$ 和 $r$。",
                  "<b>求根公式與判別式</b>：分別求兩個二次因式的判別式 $\\Delta$，排除無實根的因式。"
            ],
      ...
      "paper": "正卷",
      "qNum": "解答題 第3題",
      "topic": "三角恆等變換與倍角/半角公式",
      "score": "8分",
      "q": "在 $\\triangle ABC$ 中，$\\sin(A + B) = 6 \\sin^2 \\frac{C}{2}$。<br>(a) 求 $\\cos C$。(4分)<br>(b) 若 $\\angle A = 45^\\circ$，求 $\\sin 2B$。(4分)",
      "knowledge": {
            "formulas": [
                  "A+B+C = 180^\\circ \\implies \\sin(A+B) = \\sin C",
                  "2\\sin^2 \\frac{C}{2} = 1 - \\cos C",
                  "\\sin^2 C + \\cos^2 C = 1, \\quad \\sin 2B = -\\cos 2C \\; (\\text{當 } 2A = 90^\\circ)"
            ],
            "points": [
                  "<b>三角形內角和誘導公式</b>：$\\sin(A+B) = \\sin(\\pi - C) = \\sin C$。",
                  "<b>二倍角降冪公式反用</b>：$6\\sin^2(C/2) = 3(1 - \\cos C)$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "\\text{AA 相似判定：兩角對應相等之兩三角形相似}",
                  "\\text{相似比：}\\frac{DE}{DF} = \\frac{DG}{DE} \\implies DE^2 = DG \\cdot DF"
            ],
            "points": [
                  "<b>公共角與已知角</b>：(a) 中 $\\angle D$ 為公共角，直接 AA 相似。",
                  "<b>等腰三角形底角相等</b>：$AB=AC \\implies \\angle B=\\angle C$, $AD=AE \\implies \\angle ADE=\\angle AED$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "k_{AM} \\cdot k_{BM} = \\frac{y}{x+2\\sqrt{2}} \\cdot \\frac{y}{x-2\\sqrt{2}} = -\\frac{1}{2} \\implies \\frac{x^2}{8} + \\frac{y^2}{4} = 1",
                  "\\text{點差法斜率積定理：} k_{PQ} \\cdot k_{OD} = -\\frac{b^2}{a^2} = -\\frac{4}{8} = -\\frac{1}{2}"
            ],
            "points": [
                  "<b>斜率積軌跡方程</b>：動點到兩對稱定點的斜率積為負常數，軌跡必為橢圓（除去與 $x$ 軸交點）。",
                  "<b>點差法秒殺弦中點斜率</b>：對於橢圓 $\\frac{x^2}{a^2} + \\frac{y^2}{b^2} = 1$，割線斜率 $k$ 與中點和原點連線斜率 $k_{OD}$ 滿足 $k \\cdot k_{OD} = -\\frac{b^2}{a^2}$。"
            ],
