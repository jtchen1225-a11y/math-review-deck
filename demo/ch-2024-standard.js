    ...
    slides: [
      ...
      "knowledge": {
            "formulas": [
                  ...
                  "3x + a \\ge 0 \\iff x \\ge -\\frac{a}{3}"
            ],
            "points": [
                  "<b>交集左端點對齊</b>：$A = [-1, 4]$，$B = [-a/3, \\infty)$，交集為 $[2, 4]$ 意味著 $-a/3 = 2$。",
                  "<b>一次不等式移項</b>：$3x \\ge -a \\implies x \\ge -a/3$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "f(x+1) - f(x) = -1 \\implies f(n) = f(0) + n(-1) = 16 - n"
            ],
            "points": [
                  "<b>遞推式轉化</b>：移項得 $f(x+1) = f(x) - 1$，自變量每增加 1，函數值減少 1。",
                  "<b>等差數列通項</b>：以 16 為首項、公差為 -1 的等差數列。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "x(y+1) - (x-1)(y-1) = (xy+x) - (xy - x - y + 1) = 2x + y - 1",
                  "2x + 4y = -1 \\implies \\Delta y = -\\frac{1}{2} \\Delta x"
            ],
            "points": [
                  "<b>多項式展開消元</b>：二次交叉項 $xy$ 相互抵消，實質為二元一次線性方程。",
                  "<b>斜率與增量關係</b>：$\\frac{\\Delta y}{\\Delta x} = -\\frac{2}{4} = -\\frac{1}{2}$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "(\\sqrt{x}-2)^5 = \\sum_{r=0}^5 \\binom{5}{r} x^{\\frac{5-r}{2}} (-2)^r",
                  "(2x-1)^4 = \\sum_{s=0}^4 \\binom{4}{s} (2x)^{4-s} (-1)^s"
            ],
            "points": [
                  "<b>分數指數匹配整數次冪</b>：若要得到 $x^1$，$(\\sqrt{x}-2)^5$ 貢獻的方次必須是整數次冪，即 $r = 1, 3, 5$。",
                  "<b>分類討論累加係數</b>：逐一匹配兩部分次數和為 1 的項。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "(x - x_0)^2 + (y - y_0)^2 = R^2",
                  "R^2 = |OP|^2 = (2-0)^2 + (3-0)^2 = 13"
            ],
            "points": [
                  "<b>圓心與半徑幾何定義</b>：動點到定點距離固定即為圓，定點為圓心 $P(2, 3)$。",
                  "<b>過原點代入展開</b>：常數項為 $x_0^2 + y_0^2 - R^2 = 0$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "\\log a^k = k \\log a, \\quad \\log a + \\log b = \\log(ab)",
                  "1 = \\log 10, \\quad \\log 20 - \\log 10 = \\log 2"
            ],
            "points": [
                  "<b>常用對數換元法</b>：全部以 $\\log 2$ 表達分子與分母。",
                  "分子：$3(-\\log 2) + 4\\log 2 = \\log 2$。分母：$\\log 4 + \\log 5 - \\log 10 = \\log\\frac{20}{10} = \\log 2$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "a_n = a_1 q^{n-1}",
                  "a_7 + a_{10} = q^5 (a_2 + a_5)"
            ],
            "points": [
                  "<b>等比數列下標間隔比值</b>：第 7 項與第 2 項相差 5 步，第 10 項與第 5 項亦相差 5 步，提公因子 $q^5$。",
                  "<b>2 的整數冪必背</b>：$2^{10} = 1024, 2^{18} = 262144$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "\\bar{x} = \\frac{\\sum x_i}{5} = 6.8 \\implies \\sum x_i = 34",
                  "\\text{中位數：5 個數據排序後的第 3 個數}"
            ],
            "points": [
                  "<b>解一元一次方程求 $n$</b>：求和列方程解出 $n$。",
                  "<b>求出具體數值後重新排序</b>：帶入 $n$ 求得所有數據再排大小。"
            ],
      ...
      "options": [
            "A. $\\frac{m^4 + 2m + 1}{2m^2}$",
            "B. $\\frac{m^4 - 1}{2m^2}$",
            "C. $\\frac{m^2}{2} + \\frac{1}{2m^2}$",
            "D. $\\frac{\\sqrt{m^2 + 1}}{2}$",
            "E. 以上皆非"
      ],
      "knowledge": {
            "formulas": [
                  "1 + \\left(\\frac{A - 1/A}{2}\\right)^2 = \\left(\\frac{A + 1/A}{2}\\right)^2",
                  "\\frac{m^4 + 1}{2m^2} = \\frac{m^2}{2} + \\frac{1}{2m^2}"
            ],
            "points": [
                  "<b>完全平方交叉項符號變換</b>：$1 + \\frac{(m^4-1)^2}{4m^4} = \\frac{4m^4 + (m^8 - 2m^4 + 1)}{4m^4} = \\frac{(m^4+1)^2}{4m^4}$。",
                  "<b>雙曲函數/勾股數通項模型</b>：本題為經典雙曲三角恆等式 $\\cosh^2 t - \\sinh^2 t = 1$ 的代數結構。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "\\frac{c}{\\sin C} = \\frac{b}{\\sin B} = 2R",
                  "a^2 = b^2 + c^2 - 2bc \\cos A \\quad \\text{或餘弦定理對 } C"
            ],
            "points": [
                  "<b>正弦定理求角 $B$</b>：$\\sin B = \\frac{b \\sin C}{c} = \\frac{7 \\cdot \\frac{4\\sqrt{3}}{7}}{8} = \\frac{\\sqrt{3}}{2}$。",
                  "<b>銳角三角形條件約束</b>：$B$ 必為 $60^\\circ$（非 $120^\\circ$）。$\\cos C = \\sqrt{1 - (4\\sqrt{3}/7)^2} = \\sqrt{1/49} = \\frac{1}{7}$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "y = a(x - x_1)(x - x_2) = a(x + 2)(x - 6)",
                  "x_{\\text{頂點}} = \\frac{x_1 + x_2}{2} = \\frac{-2 + 6}{2} = 2, \\quad y_{\\max} = y(2)"
            ],
            "points": [
                  "<b>交點式設方程</b>：已知兩與 $x$ 軸交點，設 $y = a(x+2)(x-6)$ 最簡捷。",
                  "<b>對稱軸與極值</b>：二次函數在對稱軸處取得頂點最值。"
            ],
      ...
      "options": [
            ...
            "E. $\\left(\\frac{\\pi}{3}, \\frac{5\\pi}{6}\\right)$"
      ],
      "knowledge": {
            "formulas": [
                  "\\cos u \\text{ 的單調遞增區間：} [2k\\pi + \\pi, 2k\\pi + 2\\pi]"
            ],
            "points": [
                  "<b>複合函數單調性</b>：令整體角 $u = x + \\frac{\\pi}{3}$，需滿足 $\\pi \\le x + \\frac{\\pi}{3} \\le 2\\pi$。",
                  "<b>解不等式求 $x$ 區間</b>：$\\frac{2\\pi}{3} \\le x \\le \\frac{5\\pi}{3}$。子區間亦為單調遞增區間。"
            ],
      ...
      "paper": "正卷",
      "qNum": "選擇題 第13題",
      "topic": "同角三角函數基本關係與一元二次三角方程",
      "score": "4分",
      "q": "若 $\\theta \\in [0, \\pi)$ 且 $1 + \\sin \\theta - 2 \\cos^2 \\theta = 0$，則 $\\theta = (\\quad)$。",
      "options": [
            "A. $\\frac{\\pi}{6}$ 或 $\\frac{5\\pi}{6}$",
            "B. $\\frac{\\pi}{3}$",
            "C. $\\frac{\\pi}{6}$ 或 $\\frac{\\pi}{3}$",
            "D. $\\frac{\\pi}{6}$ 或 $\\frac{\\pi}{2}$",
            "E. $\\frac{\\pi}{3}$ 或 $\\frac{\\pi}{2}$"
      ],
      "knowledge": {
            "formulas": [
            ...
            "points": [
                  "<b>化一元三角函數</b>：利用平方和公式將 $\\cos^2 \\theta$ 化為 $1 - \\sin^2 \\theta$。",
                  "<b>定義域檢驗</b>：$\\theta \\in [0, \\pi)$ 時，$\\sin \\theta \\ge 0$，故 $\\sin \\theta = -1$ 無解！"
            ],
      ...
      "knowledge": {
            "formulas": [
                  ...
                  "x^4 + \\frac{1}{x^4} = \\left(x^2 + \\frac{1}{x^2}\\right)^2 - 2 = 47"
            ],
            "points": [
                  "<b>兩邊同除以 $x$</b>：$x^2 - 3x + 1 = 0 \\implies x - 3 + \\frac{1}{x} = 0 \\implies x + \\frac{1}{x} = 3$。",
                  "<b>二重平方階梯遞推</b>：一次平方求二次，二次平方求四次。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "f(-x) = f(x) = f(|x|)",
                  "\\text{偶函數在 } (-\\infty, 0) \\text{ 遞減} \\iff \\text{在 } (0, +\\infty) \\text{ 遞增}"
            ],
            "points": [
                  "<b>偶函數單調性反轉</b>：對稱區間單調性相反，在正半軸嚴格單調遞增。",
                  "<b>取絕對值比大小</b>：$f(a) = f(|a|)$，只需比較三個自變量絕對值的大小。"
            ],
      ...
      "paper": "正卷",
      "qNum": "解答題 第1題",
      "topic": "超幾何分佈概率與離散型隨機變量數學期望",
      "score": "8分",
      "q": "10 件產品中含有 3 件次品。現隨機抽出 4 件。<br>(a) 求抽出至少有 2 件次品的概率。(4分)<br>(b) 求抽出的次品數的數學期望。(4分)",
      "knowledge": {
            "formulas": [
                  "P(X = k) = \\frac{\\binom{3}{k}\\binom{7}{4-k}}{\\binom{10}{4}}",
                  "E(X) = n \\frac{M}{N} = 4 \\times \\frac{3}{10} = 1.2"
            ],
            "points": [
                  "<b>超幾何分佈模型</b>：不放回抽樣，總數 $N=10$，次品 $M=3$，抽取 $n=4$。",
                  "<b>至少2件次品互斥拆解</b>：$P(X \\ge 2) = P(X = 2) + P(X = 3)$。"
            ],
      ...
      "knowledge": {
            ...
            "points": [
                  "<b>同角三角函數商數關係</b>：由 $\\cos\\beta$ 求 $\\sin\\beta$ 與 $\\tan\\beta$。",
                  "<b>拆角/配角技巧</b>：將 $\\alpha + 2\\beta$ 視為 $(\\alpha + \\beta) + \\beta$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "a_2^2 = a_1 a_5 \\iff (3+d)^2 = 3(3+4d)",
                  "S_n = \\frac{n[2(3) + (n-1)(3)]}{2} = \\frac{3n(n+1)}{2}"
            ],
            "points": [
                  "<b>等比中項列方程</b>：$(a_1+d)^2 = a_1(a_1+4d)$ 解出公差 $d$。",
                  "<b>二次不等式整數解</b>：解 $S_n \\ge 12n + 36$ 求滿足條件的最小正整數。"
            ],
      ...
      "paper": "正卷",
      "qNum": "解答題 第4題",
      "topic": "分段絕對值函數與閉區間二次函數最值",
      "score": "8分",
      "q": "設函數 $f(x) = a - |x - 3| - |x - 7|$。<br>(a) 當 $a = 8$ 時，求解不等式 $f(x) \\ge 0$。(4分)<br>(b) 如果 $g(x) = x f(x)$ 在閉區間 $[-1, 1]$ 上有最小值 -1，求 $a$ 的值。(4分)",
      "knowledge": {
            "formulas": [
                  "|x-3| + |x-7| = \\begin{cases} 10 - 2x, & x < 3 \\\\ 4, & 3 \\le x \\le 7 \\\\ 2x - 10, & x > 7 \\end{cases}",
                  "x \\in [-1, 1] \\implies |x-3| = 3-x, \\; |x-7| = 7-x \\implies f(x) = a - (10 - 2x) = 2x + a - 10"
            ],
            "points": [
                  "<b>零點分段法去絕對值</b>：以 $x=3, 7$ 為分界點分段討論。",
                  "<b>閉區間二次函數求最值</b>：$g(x) = x(2x + a - 10) = 2x^2 + (a-10)x$ 的對稱軸為 $x_0 = -\\frac{a-10}{4}$。分對稱軸位置討論最小值。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "e = \\frac{c}{a} = \\frac{\\sqrt{10}}{2} \\implies c^2 = \\frac{5}{2}a^2, \\; b^2 = c^2 - a^2 = \\frac{3}{2}a^2",
                  "OP \\perp OQ \\iff x_1 x_2 + y_1 y_2 = 0"
            ],
            "points": [
                  "<b>待定係數求標準方程</b>：代入點 $A(2\\sqrt{2}, 3)$ 求出 $a^2, b^2$。",
                  "<b>直線聯立與韋達定理</b>：將 $y = x + m$ 代入雙曲線方程，利用 $x_1 x_2 + (x_1 + m)(x_2 + m) = 2x_1 x_2 + m(x_1 + x_2) + m^2 = 0$ 解 $m$。"
            ],
