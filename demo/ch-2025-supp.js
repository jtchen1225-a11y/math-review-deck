...
(function() {
  ...
  DECK.push({
    ...
    slides: [
      ...
      "knowledge": {
            "formulas": [
                  "V = \\frac{1}{3} S_{\\text{底}} h = \\frac{1}{3} a^2 VM",
                  "\\text{面面垂直性質定理：} \\alpha \\perp \\beta, \\; l \\subset \\alpha, \\; l \\perp (\\alpha \\cap \\beta) \\implies l \\perp \\beta",
                  "\\text{二面角求法：三垂線定理或二面角平面角定義}"
            ],
            "points": [
                  "<b>高之求法</b>：$\\triangle VAD$ 為正三角形，中線 $VM \\perp AD$。因面 $VAD \\perp$ 面 $ABCD$，故 $VM \\perp$ 底面 $ABCD$，$VM$ 即為錐體之高！",
                  "<b>二面角構造</b>：取 $VD$ 中點 $P$，連 $AP, BP$，證明 $AP \\perp VD, BP \\perp VD$，則 $\\angle APB = x$ 即為二面角之平面角。"
            ],
      ...
      "paper": "附加卷",
      "qNum": "第2題",
      "topic": "微積分 · 函數導數、極值拐點、動態切線與定積分面積",
      "score": "20分",
      ...
      "knowledge": {
            "formulas": [
                  ...
                  "S = \\int_a^b [y_{\\text{上}} - y_{\\text{下}}] dx = \\int_{-2}^1 [(-3x^2 - 3x + 6)] dx = \\frac{27}{2}"
            ],
            "points": [
                  "<b>因式分解求零點</b>：$x^3 + 3x^2 - 4 = (x-1)(x+2)^2 = 0$。",
                  "<b>一階導數判別極值</b>：令 $f'(x) = 0$ 得駐點 $x = -2, 0$。$f''(-2) = -6 < 0$（極大值 0），$f''(0) = 6 > 0$（極小值 -4）。",
                  "<b>二階導數求拐點</b>：令 $f''(x) = 0 \\implies x = -1$，拐點坐標為 $(-1, -2)$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "k = \\frac{y_2 - y_1}{x_2 - x_1}, \\quad a^2 = b^2 + c^2",
                  "\\text{直線與橢圓相切：聯立二次方程判別式 } \\Delta = 0"
            ],
            "points": [
                  "<b>焦點在軸上</b>：右焦點 $F_1(c, 0)$ 必然在 $x$ 軸上，故直線 $F_1P$ 與 $x$ 軸的交點即為 $F_1$。",
                  "<b>頂點與常數關係</b>：點 $A(0, -1)$ 在橢圓上，直接得出 $b = 1$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "z^3 = 8 \\implies z = 2\\left(\\cos\\frac{2k\\pi}{3} + i\\sin\\frac{2k\\pi}{3}\\right)",
                  "1 - e^{i\\alpha} = -2i e^{i\\alpha/2} \\sin\\frac{\\alpha}{2}",
                  "\\sum_{k=1}^6 \\sin(k\\theta) = \\frac{\\sin\\frac{7\\theta}{2} \\sin 3\\theta}{\\sin\\frac{\\theta}{2}} = 0"
            ],
            "points": [
                  "<b>複數立方根</b>：8 的三個立方根為 $2, -1 + \\sqrt{3}i, -1 - \\sqrt{3}i$。由 $b \\ne 0$ 排除實數根 2。",
                  "<b>等比級數求和引理</b>：利用歐拉公式提出半角因子，化為實數正弦之比。",
                  "<b>三角積化和差/虛部提取</b>：將三角和視為複數幾何級數之虛部求解。"
            ],
      ...
      "paper": "附加卷",
      "qNum": "第5題",
      "topic": "線性代數 · 矩陣行列式因式分解與線性方程組解的判定",
      "score": "20分",
      ...
      "knowledge": {
            "formulas": [
                  "|C| = -(a+b+c)[(a-b)^2 + (b-c)^2 + (a-c)^2]",
                  "D = \\det \\begin{pmatrix} k & 2 & -1 \\\\ 0 & k & 1 \\\\ k & 3 & 0 \\end{pmatrix} = k(k-1)",
                  "\\text{克萊姆法則：} D \\ne 0 \\iff \\text{方程組有唯一解}"
            ],
            "points": [
                  "<b>行列式初等行變換技巧</b>：行相加消元提公因式 $(a+b+c)$。",
                  "<b>實數平方和恆正性</b>：$(a-b)^2 + (b-c)^2 + (a-c)^2 = 0 \\iff a = b = c$。",
                  "<b>增廣矩陣秩判定方程組解</b>：當係數行列式 $D = 0$ 時，討論無窮解之相容條件。"
            ],
