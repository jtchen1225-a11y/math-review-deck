...
(function() {
  ...
  DECK.push({
    ...
    slides: [
      ...
      "knowledge": {
            "formulas": [
                  "\\text{菱形面積：} S_{ABCD} = |AB|^2 \\sin \\angle DAB = (2\\sqrt{2})^2 \\sin 60^\\circ = 4\\sqrt{3}",
                  "V = \\frac{1}{3} S_{ABCD} \\cdot |EF| = \\frac{1}{3}(4\\sqrt{3})(\\sqrt{2}) = \\frac{4\\sqrt{6}}{3}"
            ],
            "points": [
                  "<b>等腰直角三角形中線</b>：$\\triangle EBC$ 是等腰直角三角形，斜邊 $BC = 2\\sqrt{2}$，中線 $|EF| = \\frac{1}{2}|BC| = \\sqrt{2}$。",
                  "<b>線面垂直判定（勾股逆定理）</b>：在 $\\triangle EFG$ 中，$|EF|^2 + |FG|^2 = (\\sqrt{2})^2 + (\\sqrt{7})^2 = 2 + 7 = 9 = |EG|^2 \\implies EF \\perp FG$。聯立 $EF \\perp BC$ 證面面垂直。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  ...
                  "f''(x) = 6x",
                  "\\text{過曲線外一點切線：設切點 } (t, f(t)), \\; m = f'(t) = \\frac{f(t) - y_0}{t - x_0}"
            ],
            "points": [
                  "<b>駐點與極值</b>：$f'(x) = 0 \\implies x = \\pm 2$。極大值 $f(-2) = 22$，極小值 $f(2) = -10$。",
                  "<b>拐點坐標</b>：$f''(x) = 0 \\implies x = 0, f(0) = 6$，拐點為 $(0, 6)$。",
                  "<b>過曲線外一點切線方程</b>：必須先設切點 $t$，列出斜率等式求解 $t$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "\\text{橢圓切線斜截式：} y = mx \\pm \\sqrt{a^2 m^2 + b^2} = mx \\pm \\sqrt{9m^2 + 4}",
                  "m_1 m_2 = \\frac{k^2 - 4}{h^2 - 9} = -1 \\implies h^2 + k^2 = 9 + 4 = 13"
            ],
            "points": [
                  "<b>判別式法導出切線公式</b>：聯立直線 $y = mx + c$ 與橢圓方程，令 $\\Delta = 0$ 得 $c^2 = 9m^2 + 4$。",
                  "<b>韋達定理求兩切線斜率積</b>：兩切線過點 $A(h, k)$，滿足 $(k - mh)^2 = 9m^2 + 4$。",
                  "<b>蒙日圓定理 (Monge's Circle)</b>：橢圓的相互垂直的切線之交點軌跡為圓 $x^2 + y^2 = a^2 + b^2$！"
            ],
      ...
      "knowledge": {
            "formulas": [
                  "z = x + yi, \\; \\bar{z} = x - yi, \\; |z| = \\sqrt{x^2 + y^2}",
                  "z - 3\\bar{z} + |z| = (x - 3x + \\sqrt{x^2+y^2}) + (y + 3y)i = (-2x + \\sqrt{x^2+y^2}) + 4yi",
                  "z_1 + z_2 = 1, \\; z_1 z_2 = 1 \\implies z_1^3 + z_2^3 = (z_1+z_2)(z_1^2 - z_1 z_2 + z_2^2) = -2"
            ],
            "points": [
                  "<b>虛實部分離法</b>：複數相等等價於實部等於實部、虛部等於虛部。",
                  "由虛部 $4y = 16 \\implies y = 4$；代入實部 $-2x + \\sqrt{x^2 + 16} = -1$ 解出 $x$。",
                  "<b>幾何單位圓向量求和</b>：$|z_1|=|z_2|=1$ 且和為 1，兩複數構成菱形，夾角為 $120^\\circ$。"
            ],
      ...
      "knowledge": {
            "formulas": [
                  ...
                  "2\\sin\\theta \\cos(2k-1)\\theta = \\sin 2k\\theta - \\sin(2k-2)\\theta",
                  "\\lim_{n \\to \\infty} \\frac{1}{n}\\sum_{k=1}^n f(x_k) = \\int_0^1 f(x) dx"
            ],
            "points": [
                  "<b>和差化積證明</b>：由加法定理展開相減即得。",
                  "<b>裂項相消求三角和</b>：兩邊同乘 $2\\sin\\theta$，各項 telescoping 正負相互抵消，只留首末項。",
                  "<b>黎曼和與定積分轉化</b>：定積分 $\\int_0^1 \\cos\\left(\\frac{2\\pi x}{3}\\right) dx = \\left[ \\frac{3}{2\\pi}\\sin\\left(\\frac{2\\pi x}{3}\\right) \\right]_0^1 = \\frac{3\\sqrt{3}}{4\\pi}$。"
            ],
